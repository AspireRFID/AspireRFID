...
import org.ow2.aspirerfid.ide.bpwme.diagram.part.BpwmeLinkDescriptor;
 ...
 * @generated
 */
public class WorkflowMapCanonicalEditPolicy extends
	...
	protected List getSemanticChildrenList() {
		...
		for (Iterator it = BpwmeDiagramUpdater
				.getWorkflowMap_1000SemanticChildren(viewObject).iterator(); it
				.hasNext();) {
	...
	private Collection refreshConnections() {
		...
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
			...
			int diagramLinkVisualID = BpwmeVisualIDRegistry
			...
			for (Iterator linkDescriptorsIterator = linkDescriptors.iterator(); linkDescriptorsIterator
					.hasNext();) {
				BpwmeLinkDescriptor nextLinkDescriptor = (BpwmeLinkDescriptor) linkDescriptorsIterator
						.next();
	...
	private Collection collectAllLinks(View view, Map domain2NotationMap) {
		...
		case WorkflowMapEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(BpwmeDiagramUpdater
						.getWorkflowMap_1000ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
		...
		case OLCBProcEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(BpwmeDiagramUpdater
						.getOLCBProc_2001ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case CLCBProcEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(BpwmeDiagramUpdater
						.getCLCBProc_3001ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case EBProcEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(BpwmeDiagramUpdater
						.getEBProc_3002ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
	...
	private Collection createConnections(Collection linkDescriptors,
		...
		for (Iterator linkDescriptorsIterator = linkDescriptors.iterator(); linkDescriptorsIterator
				.hasNext();) {
			final BpwmeLinkDescriptor nextLinkDescriptor = (BpwmeLinkDescriptor) linkDescriptorsIterator
					.next();
