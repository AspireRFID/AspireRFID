...
/**
 * @author Lo�c Schmidt e-mail: loic.schmidt@lifl.fr
 *
 ...
 * All needed files are now include in the jar file.
 * The constructor with a parameters is deprecated but kept for 
 * 	  compatibility.
 *
...
public class GS1TDTEngine {
     ...
     *	GS1TDTEngine - constructor for a new GS1 translation engine
     *	 
     *  <p>When the class GS1TDTEngine is constructed, the Tag Data
     *  Translation definition files for the various GS1 application
     *  indentifiers are found in the jar file
     */
	...
	public String convert(String input, Map<String,String> inputParameters) throws TDTException {
		// check if input is GS1_AI_ENCODING or LEGACY
		boolean inputIsGs1AIEncoding=(input.indexOf("]")==0);
		boolean inputIsLegacy=(input.indexOf("=")!=-1);
		if (!inputIsLegacy && !inputIsGs1AIEncoding) return input;

		String outputData=input;

		// convert
		if (inputParameters.get("outputFormat").compareTo("GS1_AI_ENCODING")==0) {
			// need the "gs1symbol" parameter
			if (!inputParameters.containsKey("gs1symbol")) throw new TDTException("No GS1 Bar Code Types symbol in parameters");
			// need "codelength" if "gs1symbol" is ]E0 or ]E3
			if (((inputParameters.get("gs1symbol").compareTo("]E0")==0) || (inputParameters.get("gs1symbol").compareTo("]E3")==0)) && !inputParameters.containsKey("codelength")) throw new TDTException("No GTIN length in parameters");
			// processing
			if (inputIsGs1AIEncoding) outputData=this.convertGS1AIEncodingToGS1AIEncoding(input.substring(0,3), input.substring(3),inputParameters);
			else outputData=this.convertLegacyToGS1AIEncoding(inputParameters.get("gs1symbol"), input,inputParameters);
		} else if (inputIsGs1AIEncoding) outputData=this.convertGS1AIEncodingToLegacy(input.substring(0,3), input.substring(3),inputParameters);
