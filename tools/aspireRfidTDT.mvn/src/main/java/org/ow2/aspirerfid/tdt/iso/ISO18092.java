 ...
 *
 *	<p>This class represents a ISO 18092 NFCID1 
 *      
 * @version Revision: 0.5 Date: 2009/11/18
 */

public class ISO18092 implements ISOSerial {
     ...
     * that rules the encoded NFCID1.
     * In 18092, there are three kind of NFCID1 : single, double or triple size NFCID1
     ...
     * for iso NFCID1 with double or triple UID (not for single UID)
     */
	private int uid0;
    /** 
    ...
    /**
     *	ISO18092 - constructor for a new ISO 18092 serial
     *	 
     *	@param input represents the UID in inbound representation
     *	
     */
	public ISO18092(String input) throws TDTException {
	...
	private String toLegacyString() {
		...
		return "iso18092"+this.iso+";mfgcode="+this.uid0+outMfgCode;
	}
	...
	private String toTagEncodingURI() {
		return "urn:iso:tag:18092"+this.iso+"-"+this.serialSize*8+":"+this.uid0+"."+this.isoSerial.toString();
	}
	...
	private String toPureIdentityURI() {
		return "urn:iso:id:18092"+this.iso+":"+this.uid0+"."+this.isoSerial.toString();
	}
	...
	private String toONSHostname() {
		return this.isoSerial.toString()+"."+this.uid0+".18092"+this.iso+".onsiso.com";
	}
