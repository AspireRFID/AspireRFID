...
public class Pattern {
	 ...
	 * @throws ImplementationException if an implementation exception occurs
	 */
	//changed by nkef@ait.edu.gr (added tagURI to the method)
	public boolean isMember(String tagPureURI, String tagURI) throws ECSpecValidationExceptionResponse, ImplementationExceptionResponse {
		String isTagMember = null;
		
		if (usage == PatternUsage.TAG) {
			return false;
		}
		
		//added by nkef@ait.edu.gr checks if the requested patters is pureURI or tagURI
		if (thirdField.equals("idpat") ) {
			isTagMember = tagPureURI;
		} else{
			isTagMember = tagURI;
		}
		
		// create pattern of usage TAG ('*' and 'X' are not allowed)
		Pattern tag = new Pattern(isTagMember, PatternUsage.TAG);
	 ...
	 * @throws ECSpecValidationException if the tag pattern is invalid
	 */
	public String getGroupName(String tagPureURI,String tagURI) throws ImplementationExceptionResponse, ECSpecValidationExceptionResponse {
		
		if (usage != PatternUsage.GROUP || !isMember(tagPureURI,tagURI)) {
			return null;
		}
	...
	private void parseDataFields(String dataFieldsString, String pattern) throws ECSpecValidationExceptionResponse {
		...
		for (int i = 0; i < dataFieldsStringArray.length; i++) {
			dataFields.add(new PatternDataField(dataFieldsStringArray[i], usage, thirdField));
		}
