...
package org.ow2.aspirerfid.ale.server.readers;
...
import org.ow2.aspirerfid.ale.server.ALE;
import org.ow2.aspirerfid.ale.server.readers.gen.LogicalReaders;
import org.ow2.aspirerfid.ale.server.readers.gen.ObjectFactory;
...
public class LogicalReaderManager {
	...
	private static final String JAXB_CONTEXT = "org.ow2.aspirerfid.ale.server.readers.gen";
	...
	public static void setProperties(String name, List<LRProperty> properties) throws NoSuchNameExceptionResponse, ValidationExceptionResponse, InUseExceptionResponse, ImmutableReaderExceptionResponse, SecurityExceptionResponse, ImplementationExceptionResponse {
		...
		//vkol
		spec.getProperties().getProperty().retainAll(properties);

		for (int i = 0; i < properties.size(); i++){
			spec.getProperties().getProperty().add(i, properties.get(i));
	 ...
	 * @throws ImplementationException whenever something goes wrong inside the implementation 
	 */
	public static void define(String name, org.ow2.aspirerfid.ale.server.readers.gen.LRSpec spec) throws DuplicateNameExceptionResponse, ValidationExceptionResponse, SecurityExceptionResponse, ImplementationExceptionResponse {
		...
		for (org.ow2.aspirerfid.ale.server.readers.gen.LRProperty prop : spec.getLRProperty()) {
	...
	public static void initializeFromFile(String loadFilePath) throws ImplementationExceptionResponse, SecurityExceptionResponse, DuplicateNameExceptionResponse, ValidationExceptionResponse {
		...
		List<org.ow2.aspirerfid.ale.server.readers.gen.LogicalReader> genLogicalReaders;
		try {
		...
		for (org.ow2.aspirerfid.ale.server.readers.gen.LogicalReader logicalReader : genLogicalReaders) {
			
			// get logical reader name
			String logName = "";
			logName = logicalReader.getName();
			org.ow2.aspirerfid.ale.server.readers.gen.LRSpec spec = logicalReader.getLRSpec();
			define(logName, spec);	
		}
	...
	public static void storeToFile(String storeFilePath) throws ImplementationExceptionResponse, SecurityExceptionResponse, DuplicateNameExceptionResponse, ValidationExceptionResponse, FileNotFoundException {
		...
		try {
			...
			for (String name : names){
				LogicalReader logRd = logicalReaders.get(name);
				LRSpec spec = logRd.getLRSpec();
				org.ow2.aspirerfid.ale.server.readers.gen.LogicalReader genLogRd = objFactory.createLogicalReader();
				genLogRd.setName(logRd.getName());
				genLogRd.setName(name);
				org.ow2.aspirerfid.ale.server.readers.gen.LRSpec genSpec = objFactory.createLRSpec();
				...
				if (genSpec.isIsComposite()){
					...
					while(it.hasNext()){
						org.ow2.aspirerfid.ale.server.readers.gen.LRProperty genProp = objFactory.createLRProperty();
