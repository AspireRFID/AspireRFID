	...
	public PatternDataField(String stringRepresentation, PatternUsage usage, String thirdField) throws ECSpecValidationExceptionResponse {
		...
		try {
			...
			if (low <= high) {
				...
				//Added by nkef@ait.edu.gr checks if pure id paterns has range value
				if (thirdField.equals("idpat") && isRange) {
					throw new ECSpecValidationExceptionResponse("Invalid URI pattern third field '" + thirdField + "'. " +
							"Range is not alowed in pure id paterns.");
				}
			} else {
