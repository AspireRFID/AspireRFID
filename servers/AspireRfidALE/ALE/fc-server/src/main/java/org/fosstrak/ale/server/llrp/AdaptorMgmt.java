	...
	private static MessageHandler defineMessageHandler () {
		...
		@SuppressWarnings("unchecked")
			public void handle(String adaptorName, String readerName, LLRPMessage msg) {
				...
				// Added by Rim Driss : version negotiation protocol
				// Manage GET_SUPPORTED_VERSION_RESPONSE message				
				try {						
					if (item.getMessageType().equals("GET_SUPPORTED_VERSION_RESPONSE") && (statusCode.getValue(statusStr) == StatusCode.M_Success)){
						Method getsupportedVersion = msg.getClass().getMethod("getsupportedVersion", new Class[0]);
						UnsignedByte supportedVersion = (UnsignedByte) getsupportedVersion.invoke(msg, new Object[0]);
						Method getcurrentVersion = msg.getClass().getMethod("getcurrentVersion", new Class[0]);
						UnsignedByte currentVersion = (UnsignedByte) getcurrentVersion.invoke(msg, new Object[0]);
						
						LOG.debug("Status of Get Supported Version Response = " + statusCode.getValue(statusStr));
						LOG.debug ("supportedVersion = " + supportedVersion);
						LOG.debug ("supportedVersion = " + currentVersion);
					}
					
				} catch (NoSuchMethodException e) {
					LOG.error ("NoSuchMethodException in GET_SUPPORTED_VERSION_RESPONSE " + e);
				} catch (InvocationTargetException e) {
					LOG.error ("InvocationTargetException in GET_SUPPORTED_VERSION_RESPONSE " + e);
				} catch (IllegalAccessException e) {
					LOG.error ("IllegalAccessException in GET_SUPPORTED_VERSION_RESPONSE " + e);
				}
				
				// Manage SET_SUPPORTED_VERSION_RESPONSE message
						
					if (item.getMessageType().equals("SET_PROTOCOL_VERSION_RESPONSE") && (statusCode.getValue(statusStr) == StatusCode.M_Success)){
						LOG.debug("Status of Set Protocol Version Response = " + statusCode.getValue(statusStr));
					}
				//-------------------------------------------------------------------------------------------------------------------------------------------	
				...
				// if reader was disconnected, redefine the ROSpec and disable the Redefine Status
				try {
